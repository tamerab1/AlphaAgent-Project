...
def _make_isolated_db() -> tuple[sessionmaker, None]:
...
def _db_override(factory: sessionmaker):
...
@pytest.fixture()
def auth_client() -> tuple[TestClient, sessionmaker]:
...
class TestProtectedEndpointSuccess:
    def test_returns_200_with_valid_token(
        self, auth_client: tuple[TestClient, sessionmaker]
    ) -> None:
