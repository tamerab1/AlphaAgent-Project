...
function AssetChip({
  ...
  return (
    ...
    >
      ...
      <span className={`font-medium tabular-nums ${pos ? "text-positive" : "text-negative"}`}>
      ...
      <div className="mx-auto flex max-w-[1600px] items-center gap-3 px-4 py-2 lg:px-6">
        ...
        <div className="flex shrink-0 items-center gap-0.5 rounded-lg border border-border bg-bg p-0.5">
          {(["crypto", "stock"] as AssetType[]).map((t) => (
            <button
              ...
              onClick={() => {
                onTypeChange(t);
                // auto-select first asset of this type if current isn't in it
                const inType = (t === "crypto" ? CRYPTO_ASSETS : STOCK_ASSETS);
                if (!inType.find((a) => a.symbol === selectedAsset)) {
                  onAssetChange(inType[0].symbol);
                }
              }}
        ...
        <div className="relative shrink-0 w-52" ref={searchRef}>
          ...
          {open && query.trim() && results.length === 0 && (
            <div className="animate-fade-in absolute right-0 top-full mt-1 w-64 rounded-xl border border-border bg-surface2 px-4 py-3 shadow-2xl">
              <p className="text-xs text-muted">No asset found for "{query}"</p>
            </div>
          )}
        </div>
      </div>
